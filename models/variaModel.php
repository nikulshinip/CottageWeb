...
function getAllTemperatureData(){
    ...
    while (mysqli_stmt_fetch($stmt)){
        $arr[$id]=array('id'=>$id, 'address'=>$address, 'temperatur'=>$temperatur, 'title'=>$title);
    }
...
//возврашает масив {id=>temperature}
function getTemperatureData(){
    global $db;
    $sql="SELECT `id`, `temperatur`
            FROM `temperatur`
                ORDER BY `id` ASC";
    $stmt=  mysqli_prepare($db, $sql) or die(exit());
    mysqli_execute($stmt) or die(exit());
    mysqli_stmt_bind_result($stmt, $id, $temperatur);
    $arr = array();
    while (mysqli_stmt_fetch($stmt)){
        $arr[$id] = $temperatur;
    }
...
function getDS2408Data(){
    global $db;
    $sql="SELECT `id`, `connect`, `din1`, `din2`, `din3`, `din4`, `dout1`, `dout2`, `dout3`
            FROM `ds2408`
                ORDER BY `id` ASC";
    $stmt=  mysqli_prepare($db, $sql) or die(exit());
    mysqli_execute($stmt) or die(exit());
    mysqli_stmt_bind_result($stmt, $id, $connect, $din1, $din2, $din3, $din4, $dout1, $dout2, $dout3);
    $arr = array();
    while (mysqli_stmt_fetch($stmt)){
        $arr[$id]=array('id'=>$id,
                        'connect'=>$connect,
